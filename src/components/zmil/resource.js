...
export default {
  template: /* html */`
    <ComponentResource
      ...
      <!-- Заголовок ресурса -->
      <template #header>
        Проект Минобороны РФ «Герои Z» <a href="https://z.mil.ru/spec_mil_oper/heroes_z.htm" target="_blank">↗</a>
      </template>

      <!-- Схема карточки -->
      <template #card="{ card }">
        <header class="card-hero-header">
          <component-card-hero-image :src="card.resources.zmil.photo" position="top"></component-card-hero-image>
        ...
        <section class="card-hero-story">
          <div class="card-hero-story pre-line">{{ card.resources.zmil.story }}</div>
        </section>
      </template>
    </ComponentResource>
  `,
